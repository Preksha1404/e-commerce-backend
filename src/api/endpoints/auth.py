...
def login(credentials: UserLogin, response: Response, db: Session = Depends(get_db)):
    ...
    res.set_cookie("access_token", access, httponly=True, secure=IS_PROD, samesite="None", max_age= 24 * 60 * 60)
    res.set_cookie("refresh_token", refresh, httponly=True, secure=IS_PROD, samesite="None", max_age= 7 * 24 * 60 * 60)
